...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        input_ = x[:,t,:]
        state, cache_ = rnn_step_forward(input_, state, Wx, Wh, b)
        h[:,t,:] = state.reshape(N,H) 
        cache += (cache_,)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        cache_ = cache[t]
        dx_, dnext_h, dWx_, dWh_, db_ = rnn_step_backward(dnext_h, cache_)
        dx[:,t,:] = dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        input_ = x[:,t,:]
        state, cell, cache_ = lstm_step_forward(input_, state, cell, Wx, Wh, b)
        h[:,t,:] = state.reshape(N,H) 
        cache += (cache_,)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        cache_ = cache[t]
        dx_, dnext_h, dnext_c, dWx_, dWh_, db_ = lstm_step_backward(dnext_h, dnext_c, cache_)
        dx[:,t,:] = dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
