...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers): # iterate through the whole hidden layers
            if i != 0:
                input_dim = hidden_dims[i-1]
            if i == self.num_layers-1:
                hidden_dims[i] = num_classes # The last layer should have output dimension of num_classes
            ...
            self.params[weight_name] = self.weight_scale * np.random.randn(input_dim, hidden_dims[i])
            ...
            if self.normalization == "batchnorm":
                ...
                self.params[scale_name] = np.ones(hidden_dims[i])
                self.params[shift_name] = np.zeros(hidden_dims[i])
