...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        H = X.dot(W1) + b1 # Shape : (N,H)
        relu = np.maximum(0, H) # Shape : (N,H)
        ...
        # in the variable lo ss, which should be a scalar. Use the Softmax           #
        # classifier loss.                                                          #
        #############################################################################
        
        # get softmax classifier loss
        scores -= np.amax(scores, axis=1)[:,None]
        exp_scores = np.exp(scores + self.eta)
        exp_sum = np.sum(exp_scores, axis=1)
        exp_y = exp_scores[np.arange(N),y]
        loss = -np.sum(np.log(exp_y/exp_sum))/N
        ...
        # get gradient for W2
        temp1 = exp_scores/exp_sum[:,None]  # Shape : (N,C)
        temp1[np.arange(N),y] -= 1
        dW2 = relu.T.dot(temp1) / N + 2 * reg * W2 # Shape : (H,C)
        grads['W2'] = dW2 
        ...
        dW1 = X.T.dot(dH) / N + 2 * reg * W1 # Shape : (D,H)
        grads['W1'] = dW1
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            # parameters to be updated : W1, W2, b1, b2
